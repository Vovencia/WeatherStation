	...
	render() {
		return (
			<Button
				variant="contained"
				color="default"
				onClick={() => this.props.cityChooseOpen()}
			>
				...
				&nbsp;
				{this.props.loaded && this.props.title
					? this.props.title
					: "Выбрать город"}
			</Button>
		);
...
function mapDispatchToProps(
	dispatch: ThunkDispatch<IStateRoot, undefined, any>
) {
	return {
		cityLoadByName: (name: string) => dispatch(cityLoadByName(name)),
		cityLoadByCoordinates: (latt: number, lang: number) =>
			dispatch(cityLoadByCoordinates(latt, lang)),
